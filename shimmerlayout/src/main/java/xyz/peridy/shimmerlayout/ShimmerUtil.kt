...
internal object ShimmerUtil {
    ...
    internal fun calculateMaskRect(width: Int, height: Int, shimmerAngle: Int): Rect {
        ...
        val rotatedTopRight = ShimmerUtil.rotatePoint(originalTopRight, shimmerAngle.toFloat(), (shimmerWidth / 2).toFloat(), (height / 2).toFloat())
        val rotatedCenterRight = ShimmerUtil.rotatePoint(originalCenterRight, shimmerAngle.toFloat(), (shimmerWidth / 2).toFloat(), (height / 2).toFloat())
        val rotatedIntersection = ShimmerUtil.getTopIntersection(rotatedTopRight, rotatedCenterRight)
        val halfMaskHeight = ShimmerUtil.distanceBetween(rotatedCenterRight, rotatedIntersection)
    ...
    internal fun reduceColorAlphaValueToZero(actualColor: Int) =
